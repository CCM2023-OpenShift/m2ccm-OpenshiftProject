...
export const useStore = create<AppState>((set) => ({
    ...
    bookings: [],
    fetchRooms: async () => {
        const response = await fetch('https://quarkus-route-gregorydhmccm-dev.apps.rm1.0a51.p1.openshiftapps.com/rooms');
        const data = await response.json();
        set({ rooms: data });
    },
    fetchEquipment: async () => {
        const response = await fetch('https://quarkus-route-gregorydhmccm-dev.apps.rm1.0a51.p1.openshiftapps.com/equipment');
        const data = await response.json();
        set({ equipment: data });
    },
